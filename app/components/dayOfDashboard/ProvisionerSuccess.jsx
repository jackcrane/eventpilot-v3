...
export const ProvisionerSuccess = ({
  name,
  pin,
  expiryIso,
  expiryTz,
  ...
  return (
    <div>
      ...
      <Typography.Text className="d-block mb-3">
        Share this PIN with the provisioner. They will need it to sign in on the
        day of your event.
      </Typography.Text>
      <div style={PIN_STYLE}>{pin}</div>
      {expiresDescription && (
        <Typography.Text className="text-muted d-block mb-3">
          Token sessions issued from this provisioner will expire around{" "}
          {expiresDescription}.
        </Typography.Text>
      )}
