...
try {
  ...
  const fnames = [
    'Alice','Amelia','Anna','Ava','Bob','Brian','Caroline','Carol','Chloe','Charlie',
    'Daniel','David','Ella','Eve','Evelyn','Frank','George','Grace','Hannah','Heidi',
    'Henry','Isabella','Ivan','Jack','Judy','Julia','Kate','Kim','Liam','Lucas',
    'Lucy','Madison','Maria','Mary','Matthew','Natalie','Oliver','Olivia','Peter',
    'Rachel','Rebecca','Richard','Quinn','Sarah','Tom','Uma','Victor','William','Xavier','Yang','Zachary'
  ];
  const lnames = [
    'Adams','Alexander','Allen','Anderson','Bailey','Baker','Barnes','Bell','Bennett','Brooks','Brown','Carter',
    'Clark','Cooper','Cox','Davis','Daniels','Edwards','Evans','Flores','Foster','Garcia','Gomez','Gonzalez',
    'Green','Hall','Harris','Hayes','Henderson','Hernandez','Hill','Howard','Jackson','James','Jenkins','Johnson',
    'Jones','Kelly','King','Lee','Lewis','Lopez','Martin','Martinez','Miller','Mitchell','Moore','Morgan','Morris',
    'Murphy','Nelson','Parker','Patterson','Perez','Peterson','Phillips','Powell','Ramirez','Reed','Richardson',
    'Rivera','Roberts','Robinson','Rodriguez','Russell','Sanchez','Sanders','Scott','Smith','Stewart','Taylor',
  ...
  faker = {
    ...
    phone: {
      number: () => `(${randomInt(200, 989)}) ${randomInt(200, 989)}-${randomInt(1000, 9999)}`
    },
    location: {
      city: () => 'Springfield',
      state: () => 'IL',
    ...
    lorem: {
      ...
      paragraph: () => 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer et porta nisl.'
    },
    helpers: {
...
const randomTimeOfDay = (baseDate) => {
  const r = Math.random();
  let h;
  if (r < 0.8) h = 13 + Math.floor(Math.random() * 8); // 1–8pm
  else if (r < 0.92) h = 8 + Math.floor(Math.random() * 4); // 8–11am
...
const buildSchedule = (total, startDate, endDate, power = 1.7) => {
  ...
  const perDayCounts = days.map((_, i) => Math.floor((weights[i] / sum) * total));
...
const resolveEventAndInstance = async (eventRef, instanceId) => {
  if (!eventRef) throw new Error('Missing --event');
  ...
  const instance = await prisma.eventInstance.findFirst({
    where: { id: instanceId, eventId: event.id },
    select: { id: true, name: true, startTime: true, endTime: true },
  });
  if (!instance) throw new Error(`Instance not found or not under event: ${instanceId}`);
...
const loadVolunteerFields = async (eventId, instanceId) => {
  return prisma.volunteerRegistrationField.findMany({
    ...
    select: {
      ...
      options: { select: { id: true, label: true, deleted: true }, orderBy: { order: 'asc' } },
    },
  });
};
...
const loadRegistrationFields = async (eventId, instanceId) =>
  prisma.registrationField.findMany({
    where: { eventId, instanceId, deleted: false },
    orderBy: { order: 'asc' },
    include: { options: { where: { deleted: false }, orderBy: { order: 'asc' } } },
  });
...
const loadUpsells = async (eventId, instanceId) =>
  prisma.upsellItem.findMany({ where: { eventId, instanceId, deleted: false } });
...
const defaultVolunteerValueFor = (field, person) => {
  ...
  if (t === 'pagebreak' || t === 'shiftpicker' || t === 'header' || t === 'info') return null;
  ...
  if (Array.isArray(field.options) && field.options.length) {
    const opt = faker.helpers.arrayElement(field.options.filter((o) => !o.deleted));
...
const maybePickSome = (arr, maxCount = 2) => {
  ...
  const c = Math.max(0, Math.min(maxCount, Math.floor(Math.random() * (maxCount + 1))));
  if (c === 0) return [];
  // shuffle copy
...
const pickPricingForDate = (list, when) => {
  ...
  const active = list.filter(
    (p) => new Date(p.registrationPeriod.startTime) <= when && when <= new Date(p.registrationPeriod.endTime)
  );
...
// --- Writers ---
const createVolunteer = async ({ eventId, instanceId, createdAt, shifts, fields, fieldMapping }) => {
  ...
  const pickCount = Math.max(0, Math.min(shifts.length, Math.floor(Math.random() * (cfg.volunteers.maxShifts - cfg.volunteers.minShifts + 1)) + cfg.volunteers.minShifts));
  if (pickCount > 0 && shifts.length > 0) {
    const shuffled = [...shifts].sort(() => Math.random() - 0.5).slice(0, pickCount);
    await prisma.volunteerShiftSignup.createMany({
      data: shuffled.map((s) => ({ formResponseId: vr.id, shiftId: s.id, createdAt })),
      skipDuplicates: true,
    });
  }
  ...
  if (existing) {
    ...
    const link = await prisma.crmPersonLink.findFirst({ where: { crmPersonId: existing.id, formResponseId: vr.id } });
    if (!link) {
...
const createParticipant = async ({ eventId, instanceId, createdAt, fields, pricingList, upsells, teams, coupons, fieldMapping }) => {
  const person = makePerson();
  ...
  const reg = await prisma.registration.create({
    data: {
      ...
      finalized: true, // mark as finalized to show up in dashboards
    },
    select: { id: true },
  });
  ...
  for (const f of fields) {
    ...
    if (val == null) continue;
    if (f.type === 'DROPDOWN') inserts.push({ registrationId: reg.id, fieldId: f.id, optionId: val, instanceId });
    else inserts.push({ registrationId: reg.id, fieldId: f.id, value: String(val), instanceId });
  }
  if (inserts.length) await prisma.registrationFieldResponse.createMany({ data: inserts });

  // Maybe attach upsells
  const addUpsells = Math.random() < cfg.participants.upsellChance;
  if (addUpsells && upsells?.length) {
    const chosen = maybePickSome(upsells, 2);
    if (chosen.length) {
      await prisma.registrationUpsell.createMany({
        data: chosen.map((u) => ({ registrationId: reg.id, upsellItemId: u.id, quantity: 1, priceSnapshot: u.price })),
        skipDuplicates: true,
      });
    }
  }
  ...
  if (Math.random() < cfg.participants.teamChance && teams?.length) {
    ...
    await prisma.registration.update({ where: { id: reg.id }, data: { teamId: team.id } });
  }

  // Maybe attach a coupon (no validation of caps/expiry here)
  if (Math.random() < cfg.participants.couponChance && coupons?.length) {
    const coupon = coupons[Math.floor(Math.random() * coupons.length)];
    await prisma.registration.update({ where: { id: reg.id }, data: { couponId: coupon.id } });
  }
...
// --- Main ---
const main = async () => {
  const { event, instance } = await resolveEventAndInstance(eventRef, instanceId);

  // Date window
  let startDate = cfg.common.startDate ? asDate(cfg.common.startDate) : addDays(instance.startTime, -cfg.common.horizonDays);
  const today = new Date();
  let endDate = cfg.common.endDate
    ? asDate(cfg.common.endDate)
    : (cfg.common.includeFuture ? new Date(instance.startTime) : (today < instance.startTime ? today : new Date(instance.startTime)));
  ...
  // Preload lookups
  const [vFields, vShifts, rFields, pricing, upsells, teams, coupons] = await Promise.all([
  ...
  // Build schedules
  const volunteerTimes = buildSchedule(cfg.volunteers.total || 0, startDate, endDate, cfg.common.rampPower);
  const participantTimes = buildSchedule(cfg.participants.total || 0, startDate, endDate, cfg.common.rampPower);
  ...
  for (let i = 0; i < volunteerTimes.length; i++) {
    ...
    if ((i + 1) % 50 === 0) console.log(`[volunteers] created ${i + 1}/${volunteerTimes.length}`);
  }
  ...
  for (let i = 0; i < participantTimes.length; i++) {
    ...
    if ((i + 1) % 50 === 0) console.log(`[participants] created ${i + 1}/${participantTimes.length}`);
  }
