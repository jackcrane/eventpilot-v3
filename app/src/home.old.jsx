import styled, { css } from "styled-components";
import { Page } from "../components/page/Page";
import { Col, Responsive, Row } from "../util/Flex";
import { useAuth } from "../hooks";
import { Button, Typography } from "tabler-react-2";
import { Browser } from "../components/browser/Browser";
import screenshot from "../assets/screenshot.png";
import { useEffect, useState } from "react";
import paddlefest from "../assets/paddlefest.png";
import swim from "../assets/swim.png";
import orsanco from "../assets/orsanco.png";
import firstcmp from "../assets/firstcmp.png";
import ispaceDay from "../assets/ispace-day.webp";
import { HideWhenSmaller } from "../components/media/Media";

import calendarPlotCropped from "../assets/calendar-plot-cropped.svg";
import everything from "../assets/everything.svg";
import todos from "../assets/todos.png";

const Container = styled.div`
  /* background-color: var(--tblr-body-bg);
  background-image: radial-gradient(
      var(--tblr-primary) 0.5px,
      transparent 0.5px
    ),
    radial-gradient(var(--tblr-primary) 0.5px, var(--tblr-body-bg) 0.5px);
  background-size: 20px 20px;
  background-position: 0 0, 10px 10px; */
`;

const Header = styled.header`
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  height: 60vh;
`;

const Title = styled.h1`
  font-size: 3rem;
  margin: 0;
  padding: 0;
  text-align: center;
  width: 60%;
  @media screen and (max-width: 700px) {
    width: 70%;
  }
  @media screen and (max-width: 600px) {
    width: 90%;
  }
  @media screen and (max-width: 400px) {
    font-size: 2.5rem;
  }
  line-height: 1.2;
  margin-bottom: 1rem;
`;

const Subtitle = styled.p`
  font-size: 1.5rem;
  margin: 0;
  padding: 0;
  text-align: center;
  width: 60%;
  @media screen and (max-width: 700px) {
    width: 70%;
  }
  @media screen and (max-width: 600px) {
    width: 90%;
    font-size: 1.2rem;
  }
  line-height: 1.2;
  color: #666;
  margin-bottom: 3rem;
`;

const BrowserContainer = styled.div`
  max-width: 800px;
  width: 90%;
  margin: auto;
  margin-top: 20px;
  box-shadow: 100px 100px 500px -50px rgba(var(--tblr-primary-rgb), 0.15),
    100px -100px 500px -50px rgba(var(--tblr-danger-rgb), 0.15),
    -100px 100px 500px -50px rgba(var(--tblr-warning-rgb), 0.15),
    -100px -100px 500px -50px rgba(var(--tblr-success-rgb), 0.15);
  border-radius: 5px;
  overflow: hidden;
  @media screen and (min-width: 500px) {
    transform: translateY(-${(props) => props.scrollPos / 5}px);
  }
  z-index: 0;
`;

const Body = styled.div`
  background-color: var(--tblr-body-bg);
  margin-top: -200px;
  z-index: 10;
  position: relative;
  min-height: 100vh;
  padding: 20px;
  border-top: 1px solid var(--tblr-border-color);
`;

const Section = styled.div`
  max-width: 800px;
  margin: auto;
  margin-bottom: 5rem;
  overflow: hidden;
`;

const SectionTitle = styled.h2`
  font-size: 1.5rem;
  margin: 0;
  padding: 0;
  text-align: left;
  line-height: 1.2;
  margin-bottom: 3rem;
  color: var(--tblr-body-color);
  margin: auto;
  margin-bottom: 1rem;
`;

const TestamonialTitle = styled.h4`
  color: #666;
  text-align: center;
  font-weight: normal;
  font-size: 1rem;
  margin-bottom: 2rem;
`;

const TestamonialImage = styled.img`
  max-width: 120px;
  max-height: 80px;
  filter: grayscale(100%);
  opacity: 0.5;
  transition: all 0.2s ease-in-out;
  &:hover {
    filter: grayscale(0%);
    opacity: 1;
  }
`;

const SectionSubtitle = styled.h5`
  background: linear-gradient(
    to right,
    rgba(var(--tblr-primary-rgb), 0.25),
    transparent
  );
  background-size: 100px 100px;
  transition: all 0.2s ease-in-out;
  width: 200px;
  text-align: left;
  padding: 5px 10px;
  border: 1px solid var(--tblr-primary);
  border-left: 4px solid var(--tblr-primary);
  border-right: none;
  background-repeat: no-repeat;
  font-size: 0.9rem;
`;

const SectionText = styled(Typography.Text)`
  text-align: left;
  max-width: 400px;
  font-size: 1.1rem;
`;

const SectionCard = styled.div`
  border: 1px solid var(--tblr-border-color);
  border-radius: 5px;
  flex: 1;
  height: 300px;
  transition: all 0.2s ease-in-out;
  min-width: 200px;
  max-width: 300px;
  position: relative;
  overflow: hidden;
  background-color: var(--tblr-gray-100);
`;

const SectionCardTitle = styled.h5`
  font-size: 1rem;
`;

const SectionCardImage = styled.img`
  position: absolute;
  top: 10px;
  left: 30px;
  z-index: 0;
  max-height: 180px;
  transition: all 0.2s ease-in-out;
  &:hover {
    top: 0;
    left: 20px;
  }
  ${(props) =>
    props.ctr &&
    `
      top: 0;
      left: 0;
      width: 100%;
      height: 140px;
      object-fit: contain;
      padding: 10px;
      &:hover {
        left: 0;
      }
    `}
`;

const SectionCardContent = styled.div`
  margin-top: 140px;
  padding: 10px;
  z-index: 10;
  position: relative;
  background-color: var(--tblr-body-bg);
  border-top: 1px solid var(--tblr-border-color);
  height: 100%;
`;

export const Home = () => {
  const { loggedIn } = useAuth();
  const [scrollPos, setScrollPos] = useState(0);
  useEffect(() => {
    window.addEventListener("scroll", () => {
      setScrollPos(window.scrollY);
    });
  }, []);

  return (
    <Container>
      <Page title="Home" allowOverflow padding={false}>
        <Header>
          <Title>Simplifying event management for every organizer</Title>
          <Subtitle>
            Empower your event with tools to improve volunteer experience,
            increase participant engagement, and grow your organization.
          </Subtitle>
          <Row gap={1}>
            {loggedIn ? (
              <Button href="/events" variant="primary">
                View Your Events
              </Button>
            ) : (
              <>
                <Button href="/login">Login</Button>
                <Button href="/register" variant="primary">
                  Register
                </Button>
              </>
            )}
          </Row>
        </Header>
        <BrowserContainer scrollPos={scrollPos}>
          <Browser>
            <img src={screenshot} alt="EventPilot Screenshot" />
          </Browser>
        </BrowserContainer>
        <Body>
          <HideWhenSmaller w={500}>
            <Section>
              <TestamonialTitle>
                Trusted by the greatest events in the world
              </TestamonialTitle>
              <Row justify="center" align="center" gap={10}>
                <TestamonialImage src={paddlefest} alt="PaddleFest" />
                <TestamonialImage src={swim} alt="Swim" />
                <TestamonialImage src={orsanco} alt="Orsanco" />
                <TestamonialImage src={firstcmp} alt="FirstCMP" />
                <TestamonialImage src={ispaceDay} alt="iSPACE Day" />
              </Row>
            </Section>
          </HideWhenSmaller>
          <Section>
            <Responsive
              justify="space-between"
              align="center"
              colAlign="flex-start"
              gap={10}
              colGap={2}
              breakpoint={600}
            >
              <Col align="flex-start">
                <SectionSubtitle>Why EventPilot?</SectionSubtitle>
                <SectionTitle>Discover the benefits of EventPilot</SectionTitle>
              </Col>
              <SectionText>
                Find out why our customers love EventPilot and how it can help
                your event reach new heights.
              </SectionText>
            </Responsive>
            <Row gap={1} wrap>
              <SectionCard>
                <SectionCardImage src={calendarPlotCropped} />
                <SectionCardContent>
                  <SectionCardTitle>Understand everything</SectionCardTitle>
                  <Typography.Text>
                    Watch your event's progress in real-time. Understand the
                    impact of email blasts, events, appearances, and more by
                    watching subsequent spikes.
                  </Typography.Text>
                </SectionCardContent>
              </SectionCard>
              <SectionCard>
                <SectionCardImage src={everything} ctr />
                <SectionCardContent>
                  <SectionCardTitle>Do everything</SectionCardTitle>
                  <Typography.Text>
                    Say goodbye to migrating to a new system every time you need
                    something new. EventPilot is fully integrated, so you only
                    need to set up your event once.
                  </Typography.Text>
                </SectionCardContent>
              </SectionCard>
              <SectionCard>
                <SectionCardImage src={todos} />
                <SectionCardContent>
                  <SectionCardTitle>Remember everything</SectionCardTitle>
                  <Typography.Text>
                    Keep track of everything that you need to get done by
                    creating todo tickets and linking them to anything else in
                    your event.
                  </Typography.Text>
                </SectionCardContent>
              </SectionCard>
            </Row>
          </Section>
          <Section>
            <SectionSubtitle>Made for you</SectionSubtitle>
            <SectionTitle>Tailor made for your event</SectionTitle>
          </Section>
        </Body>
      </Page>
    </Container>
  );
};
