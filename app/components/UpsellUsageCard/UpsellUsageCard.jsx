...
export const UpsellUsageCard = () => {
  ...
  const { total, withUpsells, percent, previous, loading } = useDashUpsellUsage({ eventId });
  ...
  const hasPrevData =
    previous && typeof previous.withUpsells === "number" && previous.withUpsells > 0;
  const description = hasPrevData
    ? `vs last: ${pct(previous.percent)} (${previous.withUpsells} of ${previous.total})`
    : undefined;
  ...
  return (
    <Card title="Upsell Adoption" style={{ width: 300, minWidth: 300 }}>
      <DataBox title="Participants with Upsells" value={value} description={description} trend={trend} />
      <Typography.Text>
